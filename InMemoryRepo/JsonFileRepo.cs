...
public class JsonFileRepository : IGearRepository
{
    private string jsonPath = "gear.json";

    private List<GearItem> Load()
    {
        if (!File.Exists(jsonPath)) return new List<GearItem>();
        string json = File.ReadAllText(jsonPath);
        return JsonSerializer.Deserialize<List<GearItem>>(json) ?? new List<GearItem>();
    }
